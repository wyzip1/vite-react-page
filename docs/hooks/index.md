...
| [useOnceState](/hooks/use-once-state) | `src/hooks/useOnceState.ts` | 首次挂载时读取 React Router `location.state`，回调后清空 `history.state.usr`。 |
| [useUnFirstEffect](/hooks/use-un-first-effect) | `src/hooks/useUnFirstEffect.ts` | 跳过首次执行，只在依赖后续变化时执行 callback。 |
...
| 只想消费一次页面跳转 state | 使用 `useOnceState`。它依赖 React Router 的 `history.state.usr` 存储约定。 |
| 跳过初始化副作用 | 使用 `useUnFirstEffect`。注意当前实现不会接管 callback 返回的清理函数。 |
...
- 这些 hooks 面向浏览器运行环境；`useModal`、`useWindowEvent`、`useOnceState` 都直接访问 `document` 或 `window`。

## 组合页说明

`docs/hooks/modal-events.md` 保留为弹窗、事件与副作用主题汇总，不再作为独立覆盖依据。逐项审核以 `use-modal`、`use-window-event`、`use-once-state`、`use-un-first-effect`、`use-sub` 为准。
